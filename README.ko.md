...
구현 단계의 우선순위·의존성과 남은 수락 기준은 [구현 마일스톤](docs/ko/roadmap.md),
PR·커밋·CI 운영 방식은 [GitHub 작업 절차](docs/ko/github-workflow.md)에 정리했다.
계획과 현재 지원 범위는 구분한다.
...
별도 [Codex 호환성 시험](tests/codex/README.ko.md)은 실제 Codex와 게이트웨이를
합성 upstream에 연결한다. 임시 시험 기준은 `0.154.0-alpha.6`이며,
`0.153.4`의 heartbeat 취소 실패와 향후 `0.154.0` 이상 안정판으로 대체할
조건은 [고정 계약](docs/ko/codex-contract.md)에 기록했다. 제품 실행을 특정
Codex 버전에 강제하거나 실제 공급자·소비자 운영 수락을 대신하지 않는다.
...
[COMMERCIAL-LICENSING.md](COMMERCIAL-LICENSING.ko.md)에 있으며 실제 대체 허가나
체결된 계약은 아니다. [기여 정책](CONTRIBUTING.ko.md)과
[제3자 고지 현황](THIRD-PARTY-NOTICES.ko.md)도 함께 확인한다.
...
소스 배포 절차는 [문서 관리](docs/ko/documentation.md)에 설명한다.

세 경로의 공통 검증과 기능 차이는 [적합성 표](docs/ko/conformance.md),
