...
def render_notices(document):
    lines = ['# 제3자 라이선스와 고지', '', '이 파일은 라이선스 기록에서 생성한다. 직접 수정하지 않는다.', '', '범위는 Cargo.lock 전체이며 모든 플랫폼·빌드·개발 의존성을 포함한다.', '아래 목록이 모두 최종 실행 파일에 링크되었다는 뜻은 아니다.', '프로젝트의 별도 계약은 제3자 구성요소의 허가·고지를 대체하지 않는다.', '원문에 다른 라이선스 선택지가 함께 실려 있어도 선택하지 않은 조건을 일괄 적용한다는 뜻은 아니다.', '', f"잠금 파일 SHA-256: `{document['cargo_lock_sha256']}`", '', f"정책 SHA-256: `{document['policy_sha256']}`", '']
    for package in document['packages']:
        lines.extend([f"## {package['name']} {package['version']}", '', f"선언: `{package['declared_license']}`", '', '선택: ' + ', '.join(f'`{x}`' for x in package['selected_licenses']), '', f"원본 패키지 SHA-256: `{package['checksum']}`", ''])
        for notice in package['notices']:
            ...
            if notice['origin'] == 'pinned_upstream':
                lines.append(f"  출처: [고정 커밋의 원문]({notice['url']})")
        lines.append('')
    return ('\n'.join(lines).rstrip('\n') + '\n').encode()
