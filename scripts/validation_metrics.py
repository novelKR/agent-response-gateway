...
def summarize(run):
    ...
    for job in run['jobs']:
        ...
        steps=[{'name':s['name'],'conclusion':s['conclusion'],'seconds':elapsed(s.get('startedAt'),s.get('completedAt'))} for s in job.get('steps',[]) if s.get('conclusion') in {'success','failure','cancelled'}]
    ...
    return dict(schema='gateway-validation-metrics/v1',run_id=run['databaseId'],head_sha=run['headSha'],event=run['event'],url=run['url'],
                status=run['status'],conclusion=run['conclusion'],attempt=run['attempt'],rerun=run['attempt']>1,required_gate_seconds=gate,
...
if __name__=='__main__':
    ...
    fields='databaseId,headSha,event,url,status,conclusion,attempt,createdAt,jobs'
