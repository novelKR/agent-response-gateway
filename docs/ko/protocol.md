...
[외부 API codec](api-codecs.md)은 명시적인 네이티브 패키지 선택으로 기존 API
계약을 구현할 수 있다. 요청 승인 검사와 코어 소유의 전송·도구 검증·사용량 기록·
연속성을 유지한다. 미지원 기능은 계속 오류이며 codec 권한으로 지원 표가 확장되지 않는다.
