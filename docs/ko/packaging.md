...
G18은 Ubuntu 24.04의 x86_64-unknown-linux-gnu와 macOS 15의
aarch64-apple-darwin 후보를 생성·검증한다. Native Rust 1.98.0과 CI의
Python 3.14를 요구한다. Gateway 런타임 의존성, 서명 권한, release tag,
공개 릴리스나 소비자 활성화는 추가하지 않는다.
...
패키지·smoke 도구가 동일 hash로 소스 archive에 포함됐는지도 확인한다.
...
[G19](release-promotion.md)가 인증된 출처와 보호된 승격을 제공한다.

| 후보 파일 | 증거 |
|---|---|
| 대상 바이너리 tar.gz | 실행 파일, 설정 예제, 제품·라이선스 문서, 커밋된 전체 Cargo 고지와 제공된 Rust 도구 고지 |
| 소스 tar.gz | Commit receipt를 가진 Git 추적 소스, 정규화된 gzip 메타데이터 |
...
| SHA256SUMS | 정확한 파일명의 모든 후보 자산과 manifest |

바이너리 archive는 일반 파일과 검토된 0644/0755 mode만 사용한다. 이름·바이트·
mode·hash가 manifest와 정확히 같아야 한다. 추가·누락, 링크, 예약 경로,
체크섬 변경과 잘못된 소스·target은 거부한다. 소스·바이너리 모두 공개 경계
검사를 적용하고 Cargo 패키지 목록도 별도로 검사한다. 소스 archive에는
...
Crate hash는 잠금 archive의 hash이며 컴파일 object의 hash가 아니다. 이름,
...
upstream CI compiler 설명은 구분한다. Gateway 패키지에는 Codex 실행 파일이 없다.
...
upstream의 세 JSON 경로, 자격 헤더 분리와 제한된 정상 종료를 검사한다.
...
상태는 올리지 않는다. 결과는 ci-required에 포함된다. PR artifact는 검토용이며
정식 승격 대상이 될 수 없다. 릴리스 후보는 검증된 main commit과 인증된
출처에서 생성해야 한다. G19는 사용자 승인 후 출처를 재검증해 같은 바이트를
승격한다. 소비자 통합, 장기 수락, 실제 모델 qualification과 배포 승인은 별도 단계다.
