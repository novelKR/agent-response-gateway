...
| 배포 도구 | 네이티브 대상 4종, 소스·고지, 의존성 목록, 서명된 태그 빌드, 자동 시험판 공개와 승인 후 정식 승급 |
...
현재 제외된 기능은 구현 경계이며 영구적인 제품 비목표가 아니다. 선택형 자격 증명
브로커, 계정 풀, 공급자 연속성과 응답 상태 서비스는 [제품 구성](index.md)을 확장할 수
있다. 명시적 계약과 구현이 필요하며 이 방향이 출시나 고정된 순서를 약속하지는 않는다.
현재 관측 프로토콜은 이 기능을 활성화하지 않는다.

지속적인 설계 원칙은 공통 의미 검증, 차이의 명시, 조용한 기능 제거 금지와 소비자의
비공개 업무 도메인에 대한 코어 의존 방지다. 사용자 시나리오를 먼저 선택하고 필요한
코어 계약·선택형 모듈·애플리케이션 통합을 정한다. 소비자 버전, 공급자·모델, 활성 확장
구성, 시나리오와 보장 수준을 함께 평가한다. 그 조합에서 도구 왕복·취소·복구·사용량
일관성이 성립해야 하며 기능 이름만으로 호환성이 입증되지는 않는다.
