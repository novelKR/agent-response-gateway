...
태그는 지원하지 않는다. 소스 커밋은 main에 포함되어야 한다. 후보는 일반 main
통합 CI와 독립적으로 정확한 태그 커밋의 전체 자격 검증을 실행하고, 통과 후에만
배포물을 빌드하거나 서명한다. 이후 main이 진행되어도 재빌드할 필요가 없다.
버전 태그를 다른 커밋으로 옮기는 것은 거부한다.
...
새 명세는 `gateway-distribution/v2`를 사용하고
`gateway-release-qualification/v1`을 포함한다. 기록은 소스 커밋, 검증 정책 해시,
전체 필수 job 결과, 지원 대상 전체와 정확한 run ID/attempt를 연결한다. 명세
서명이 이 기록도 포함한다. 검증 시 대응 소스의 정책과 해당 후보 실행에서 완료한
자격 검증 job을 확인한다. 기록 누락, 형식 혼합이나 대상별 기록 불일치는 실패한다.
새 배포물의 `pack` 명령에는 `--qualification`이 필요하다.

기존 `gateway-distribution/v1` 파일도 기존 형식 경로로 검증할 수 있다. 해당
소스 main CI의 플랫폼·시나리오 행렬을 포함한 전체 선행 검사에 실제 성공 결과가
있어야 하며 일반 workflow 성공만으로는 충분하지 않다. 원래 바이트와 서명은
보존한다. 자격을 갖춘 배포물을 이미 생성한 workflow를 복구할 때 새 형식 판독기를
제거하지 말고 후보 생성을 일시 중단한다.
