...
IR은 소비자에 종속되지 않는 의미 계약이다. 요청 의미(항목·도구·제약), 실행
계획(경로·프로필·브리지), 실행 문맥(시도 정체성·시간 제한·활성 확장)을 구분한다.
이는 책임의 구분이며 새로운 공개 타입이나 스키마를 추가하는 설명이 아니다.

| 보존 수준 | 계약 |
|---|---|
| 직접 보존 | 불필요한 재작성 없이 대응 API 기능 사용 |
| 변환을 통한 보존 | 명시적 매핑을 적용하고 약속한 관계를 검증 |
| 근사 | 다른 보장을 밝히고 명시적으로 선택; 여기서 새 근사 모드를 추가하지 않음 |
| 미지원 | 필요한 의미를 보존할 수 없으면 거부 |

`Native / Bridged / Unsupported`는 허용 판정과 구현 선택을 나타내며 독립적인
동등성 증명은 아니다. 사용자 정의 문자열과 도구 ID의 복원은 정확히 검사할 수
있다. 패치 문법 검사는 모델의 제한된 생성을 강제하지 않으며, 지시를 합치는 것만으로
역할 우선순위 보존이 증명되지 않는다. 선언된 [Messages 한계](messages.md)와
[제품 개요](index.md)를 참조한다. 미지 필드 보존은 다른 API로의 변환 허가가 아니다.
...
선택한 편집 계약은 결정적 문맥 편집·독립 작업 묶음·직접 패치 envelope 정규화도
지원한다. 필수 입력과 제한은 편집 계약을 참조하며 파일 일치 검사와 실행 결과는
호스트가 소유한다.
