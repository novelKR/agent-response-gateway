...
공개 저장소는 범용 API·실행·배포 계약을 관리한다. 현재 동작, 사용 조건과
제한을 먼저 설명한다. 내부 마일스톤·PR 표지 대신 기능 이름을 쓰고, 필요한
전문 용어는 설명한다. 개발 경과와 승인 논의는 작업 기록에 둔다. 닫힌 이슈나
Git 이력을 찾아보지 않아도 문서를 이해할 수 있어야 한다. 특정 소비자의 이름,
...
압축파일 검사도 예약 경로와 링크·특수 파일을 거부하며, 선택적 사전이 있으면
...
`.codex/`는 개인 실행 설정용 예약 경로다. index·이력·tree ref·압축파일에
...
연결된 worktree는 저장소 ref를 공유하므로 변경을 worktree로 옮겨도 로컬 snapshot
ref에서 검사가 격리되지 않는다. 공개 후보 검증에는 공개 원격의 전체 이력을 가진
별도 clone을 만들고 정확한 후보 diff를 적용한다. 변경하지 않은 검사기로 후보와
공개 이력을 검사하고 원래 checkout의 결과와 범위를 구분해 기록한다. 통과를 위해
로컬 ref를 삭제하거나 예약 경로 규칙을 완화하지 않는다.
...
페이지 복사 버튼은 현재 언어의 유지보수 Markdown 원문을 복사하며 제목, 코드
블록과 원래 링크를 보존한다. 복사 성공이나 클립보드 권한 오류를 표시한다.
상단 메뉴에서 같은 문서의 언어를 전환하며 호환 앵커를 유지한다.
...
자산을 검증한다. 빌드 명세는 게시 파일 각각의 SHA-256과 작업 트리의 변경
...
언어 전환, 기존 앵커와 404 페이지를 확인한다. 페이지 복사가 현재 언어의 Markdown
원문을 복사하고 클립보드 처리 결과를 표시하는지 점검한다. 임시 팔레트와 간격 변경이 탐색,
...
공개 결정이 필요하다. 이 결정에 소스 commit, 빌드 명세와 웹 고지를 제공한다.
