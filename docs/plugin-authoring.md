...
This specification describes the existing installable native roles. A plugin can be built in an independent repository and in any language that can implement the process and byte contracts below. Installing a compatible executable does not rebuild the gateway. No Rust ABI or gateway library import is required. Legacy roles retain their contracts; codec API subsets use the explicit capability contract below. Provider execution additionally requires explicit activation, grants and a model route.
...
Provider packages declare `gateway-provider/v1`, the same payload grants, `provider-request-memory/v1`, empty `apis`, and requirements exactly `provider_ipc_v1` and `responses_output_validation`. They additionally require `provider_protocol` matching `[a-z][a-z0-9._-]{0,63}/v[1-9][0-9]{0,5}`. Codec packages forbid that field, including null. Provider declarations may be inspected, installed and inventoried without execution. Explicit activation and model routing enable the [provider runtime](provider-plugins.md); declared features must match Ready and each admitted request. No provider role messages or new supplier routing are accepted through codec v3.
