<a id="license-policy-and-notice-records"></a>

# 라이선스 정책과 고지 기록

[English](README.md) | [한국어](README.ko.md)

프로젝트는 [라이선스 정책](../COMMERCIAL-LICENSING.ko.md)에 따라
**AGPL-3.0-only와 별도 상용 라이선스**를 사용한다. 이 문서는 어느 배포 방식을
선택하든 필요한 의존성 라이선스와 원래 고지의 검사·묶음 생성 절차를 설명한다.

<a id="managed-evidence"></a>
...
내용 해시로 고지의 중복 저장을 줄이고 패키지별 버전과 출처를 보존한다.
정책·선택·원문 변경은 함께 검토한다. 이전 고지 묶음을 재현하는 데 필요한
원문은 유지한다.
...
<a id="prepare-tools-and-sources"></a>
...
<a id="check-refresh-and-bundle"></a>
...
- `bundle`은 같은 검사를 수행한 뒤 새 디렉터리 또는 빈 디렉터리에 고지·원문·
  정책·의존성 기록·해시 명세를 생성한다. 기존 파일은 덮어쓰지 않는다.
...
<a id="evaluation-and-special-originals"></a>
...
도구가 읽는 풀린 소스도 잠금 체크섬이 일치하는 압축파일과 바이트를 대조한다.
...
<a id="publication-boundary-and-distribution"></a>
...
고지 묶음은 [공개 경계 검사](../docs/ko/documentation.md) 후 배포 자료에 포함한다.
Cargo 목록 외에 시스템 라이브러리·컨테이너 패키지·번들 실행 파일은 실제
산출물별로 조사한다. 자세한 절차는 [배포 안내](../docs/ko/release.md)를 따른다.

로컬 게이트웨이·사용량 계약·Recorder·관리·관리 실행 패키지는 루트 프로젝트 라이선스를
공유한다. 정확한 로컬 경로를 검사하며 제3자 AGPL 예외가 아니다. foldhash의
Zlib 조건을 의존성 선택에 포함하고 잠금된 소스의 원문 고지를 보존한다.
