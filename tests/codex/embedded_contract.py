...
def validate_manifest(value):
    ...
    require(value["schema"] == MANIFEST_SCHEMA and value["client_api"] == "responses" and value["lifecycle"] == "host-supervised-process/v1", "Unsupported manifest contract")
    ...
    require(isinstance(configuration, dict) and set(configuration) == {"listen", "source_url", "local_token_env", "upstream_credential_references", "limits", "routes"}, "Invalid configuration projection")
...
def parse_ready_line(line, manifest):
    ...
    require(ready["schema"] == READY_SCHEMA and ready["manifest_schema"] == manifest["schema"] and ready["event"] == "ready", "Unsupported readiness contract")
